...
struct AudioPlayerView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UIView {
        ...
        // Wait for the player item to be ready, then seek to start time
...
struct AudioPreviewView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            // Hidden audio player (no visual representation needed)
            AudioPlayerView(
                url: audioURL,
...
struct AudioTimeRangeSelector: View {
...
#Preview {
    VStack {
        Text(NSLocalizedString("audio_preview_component", comment: "Audio preview component placeholder"))
        Text(NSLocalizedString("need_audio_url_preview", comment: "Need audio URL for preview"))
    }
}